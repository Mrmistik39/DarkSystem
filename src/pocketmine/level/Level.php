...
class Level extends TimeValues implements ChunkManager, Metadatable{
	...
	public function generateChunkCallback($x, $z, Chunk $chunk){
		if($this->closed){
			return;
		}
	...
	public function setChunk($x, $z, Chunk $chunk, $unload = true){
		$index = Level::chunkHash($x, $z);
		if($unload){
			...
			foreach($this->chunkSendQueue as $index => $players){
				...
				foreach($players as $player){
					if($player->isConnected() && isset($player->usedChunks[$index])){
						...
						if(advanced_cache === true){
							$playerIndex = $player->getPlayerProtocol();
				...
				if($protocols !== []){
					...
					$task = $this->provider->requestChunkTask($x, $z, $protocols);
